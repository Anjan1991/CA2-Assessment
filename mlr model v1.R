...
new_sleep_df$Respiration_Rate <- sqrt(new_sleep_df$Respiration_Rate)
new_sleep_df$Body_Temp <- sqrt(max(new_sleep_df$Body_Temp + 1) - new_sleep_df$Body_Temp)
new_sleep_df$Limb_Movement <- sqrt(new_sleep_df$Limb_Movement)
new_sleep_df$Blood_Oxy_Level <- sqrt(max(new_sleep_df$Blood_Oxy_Level + 1) - new_sleep_df$Blood_Oxy_Level)
new_sleep_df$Sleeping_Hours <- sqrt(new_sleep_df$Sleeping_Hours)
new_sleep_df$Heart_Rate <- sqrt(new_sleep_df$Heart_Rate)
...
confint(heart_rate_model)

new_sleep_df <- subset(new_sleep_df, select = -c(Heart_Rate, Sleeping_Hours))
new_sleep_df
set.seed(1)
no_rows_data <- nrow(new_sleep_df)
sample_data <- sample(1:no_rows_data,
...
summary(powerTransform(training_data$Body_Temp))

sqrt_transform_Body_Temp <- sqrt(training_data$Body_Temp)
training_data$Body_Temp_sqrt <- sqrt_transform_Body_Temp
fit_model1 <- lm(Body_Temp ~
                  Respiration_Rate +
                  Limb_Movement +
                  Blood_Oxy_Level,
                data = training_data)

fit_model2 <- lm(Body_Temp_sqrt ~
                  Respiration_Rate +
                  Limb_Movement +
                  Blood_Oxy_Level,
                data = training_data)
